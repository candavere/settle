...
(async () => {
    ...
    await page.evaluate(() => window.scrollTo(0, document.getElementById('section-dfs').offsetTop + 400));
    await page.waitForTimeout(400);

    const section = await page.evaluate(() => {
        const s = document.getElementById('section-dfs');
        const stage = s.querySelector('.grid-stage');
        const field = document.getElementById('dfs-field');
        ...
        return {
            ...
            hasDFS: s.classList.contains('algorithm-section--dfs'),
    ...
    check('dfs has act-iii class', section.hasActIII);
    check('dfs has path class', section.hasPath);
    check('dfs has dfs class', section.hasDFS);
    ...
    // Keyframes: path-cell rules only (DFS has no halo/tilt overlay)
    const css = await page.evaluate(() => {
        const sheet = document.getElementById('dfs-path-keyframes');
    ...
    check('dfs path keyframes injected', css.hasSheet);
    ...
    const correctness = await page.evaluate(() => {
        ...
        return {
            dfsVisited: dfsResult.visited.length,
            dfsPathLen: path.length,
    ...
    check('DFS path is valid connected sequence', correctness.pathValid);
    check('DFS path has no duplicates', correctness.noDuplicates);
    check('DFS path avoids walls', correctness.pathNoWalls);
    check('DFS path reaches end cell', correctness.reachesEnd);
    check('DFS path length > 0', correctness.dfsPathLen > 0, String(correctness.dfsPathLen));
    check('DFS path is longer than the shortest on this maze', correctness.dfsPathLen > correctness.bfsPathLen,
        `dfs=${correctness.dfsPathLen} bfs=${correctness.bfsPathLen}`);
    ...
    const motion = await p2.evaluate(() => {
        ...
        const visitedCells = Array.from(document.querySelectorAll('#dfs-field .path-cell--visited'));
